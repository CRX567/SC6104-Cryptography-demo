...
算法核心思想是通过函数迭代与循环检测，在有限空间中找到两个不同输入，使它们的截断哈希值相同。
该算法能在保持时间复杂度不变的前提下，将空间复杂度从指数级降低为常数级，是理解哈希碰撞概率和随机函数性质的重要实验算法。
...
生日攻击通过随机生成大量输入并保存其哈希值，然后检测是否出现重复。
...
> 在一个有限集合中不断迭代一个确定性函数，最终必然出现重复（循环）。
...
1. Pollard’s Rho 算法通过函数迭代与循环检测，在常数空间内实现哈希碰撞搜索；
2. 截断操作使实验在合理时间内可执行，不影响算法原理；
3. 实验结果验证了哈希碰撞的概率规律（2^(m/2) 级别）；
4. 算法展示了“有限随机过程必然循环”的数学思想。
...
2. R. P. Brent, An improved Monte Carlo factorization algorithm, BIT, 1980.
3. Menezes, van Oorschot, Vanstone, Handbook of Applied Cryptography, 1996.
4. Wikipedia: Pollard’s rho algorithm
